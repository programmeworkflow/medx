// Sync chunked: pega a empresa com ultimo_sync mais antigo (ou nulo) e
// sincroniza só ela. Cron roda de hora em hora — ciclo completo de N
// empresas leva N horas, mas cada execução cabe em <60s (limite Vercel).
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: cert } = await sb
      ...
      .eq("ativo", true)
      .maybeSingle();
    if (!cert) return res.status(200).json({ ok: false, skipped: true, reason: "Sem certificado ativo" });
    ...
    // 2. Pega a empresa ativa com ultimo_sync mais antigo (ou null)
    // Ordem: nulls primeiro (nunca sincronizadas), depois mais antigas
    ...
    const stats: any = {
      empresa: { id: empresa.id, cnpj: cnpj14, nome: empresa.nome },
      cpfsConsultados: 0,
      idsRetornados: 0,
      eventosBaixados: 0,
      atualizacoes: 0,
    ...
    // 3. CPFs cadastrados pra essa empresa
    const { data: cpfs } = await sb
      .from("esocial_funcionarios")
      .select("cpf, nome, situacao")
      .eq("empresa_cnpj", cnpj14);
    if (!cpfs?.length) {
      // Atualiza timestamp mesmo sem CPFs pra rotacionar
      await sb.from("esocial_empresas_sync")
        .update({ ultimo_sync: new Date().toISOString() })
        .eq("id", empresa.id);
      return res.status(200).json({ ok: true, stats: { ...stats, motivo: "Sem CPFs cadastrados" } });
    }
    ...
    for (const f of cpfs) {
      try {
        ...
        if (r.status !== 200) {
          stats.erros.push({ cpf: f.cpf, erro: `HTTP ${r.status}` });
          continue;
        }
        ...
        stats.erros.push({ cpf: f.cpf, erro: e?.message });
      }
    }

    // 4. Download em lotes de 10
    for (let k = 0; k < idsParaBaixar.length; k += 10) {
      ...
      try {
        ...
        stats.erros.push({ erro: `download: ${e?.message}` });
      }
    }

    // 5. Atualiza ultimo_sync
    await sb
      .from("esocial_empresas_sync")
      .update({ ultimo_sync: new Date().toISOString() })
      .eq("id", empresa.id);
